...
const AuthProvider = (props: any) => {
  ...
  async function authenticate({ username, password }, cb) {
    ...
    const res = await fetch(process.env.REACT_APP_LARAVEL_API_URL + '/oauth/token', options);
  ...
  async function signout(cb) {
    ...
    const res = await fetch(process.env.REACT_APP_LARAVEL_API_URL + '/api/admin/v1/me/logout', options);
